...
class Wechat
{
    ...
    {
        if (empty($config)) {
            $config = Container::instance()->get('config')->get('wechat');
        }
    ...
    {
        // 先判断是否存在缓存
        $cache = Container::instance()->get('cache')->get('access_token');
        if ($cache) {
            return $cache;
        }
        ...
        // 缓存
        Container::instance()->get('cache')->set('access_token', $res['access_token'], $res['expires_in']);
    ...
    {
        // 先判断是否存在缓存
        $cache = Container::instance()->get('cache')->get('jsapi_ticket');
        if ($cache) {
            return $cache;
        }
        ...
        // 缓存
        Container::instance()->get('cache')->set('ticket', $res['ticket'], $res['expires_in']);
        return $res['ticket'];
    }
