...
* @version v1.2 移除CMD配置文件，由setConfig方法导入
*/
class Http
{
    /**
     ...
     * @return  $result
     */
    public static function excuteUrl($url, $data = array(), $type = 'get', $toJson = false, $timeOut = 2)
    {
        //post请求
        if($type == 'post')
        {
     ...
     * @return  $result;
     */
    public static function excuteCMD($serverName, $data = array(), $type = "get", $toJson = false, $timeOut = 2, $cache = false, $cacheTime = 60)
    {
        ...
        $uri = self::arrToUri($data);
        $url = $server['url'].$uri;
        $result = self::sendRquest($url, array(), $type, $toJson, $timeOut);
        //缓存数据
    ...
    public static function getConfig()
    {
        if(empty(self::$config))
        {
            $errorMsg = "HttpError : The config is not found！";
            self::errorQuit($errorMsg);
        }
    ...
    private static function sendRquest($url,  $data = array(), $type = 'get', $toJson = false, $timeOut = 0)
    {
        ...
        // 记录请求日志
        $log = 'url => '. $url . PHP_EOL . var_export($html, true);
        Container::get('log')->curl($log)->save();
        if($html == false || curl_getinfo($ch, CURLINFO_HTTP_CODE) != 200){
            $error_log = 'url => '. $url . PHP_EOL 
                        . 'status => ' . curl_getinfo($ch, CURLINFO_HTTP_CODE) . PHP_EOL 
                        . var_export(curl_error($ch), true);
            Container::get('log')->curl($error_log)->save();
        }
        // 关于请求句柄
        curl_close($ch);
