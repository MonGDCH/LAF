...
class OAuth
{
	 ...
	 * @return [type]       [description]
	 */
	public function createToken($info)
	{
		...
		$info = [
            ...
            'sign'	=> $this->sign($key, $info, $_SERVER['REQUEST_TIME'])
		];
	...
	{
		$key = $this->config['token_key'];
        $life = $this->config['token_life'];

		$ticket = $this->parseTicket($token);
        $info = json_decode($ticket, true);
        if(!$info || !isset($info['info']) || !isset($info['ctime']) || !isset($info['sign']) || $this->sign($key, $info['info'], $info['ctime']) != $info['sign']){
            $this->error = '无效的Token';
            return false;
        }
     ...
     * @param  [type] $info  用户标识
     * @param  [type] $ctime 创建时间,作为随机因子
     * @return [type]        [description]
     */
    protected function sign($key, $info, $ctime)
    {
    	return md5($key . $info . $ctime);
    }
}