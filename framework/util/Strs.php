...
class Strs
{
     ...
     * @return  string
     */
    public static function stringEncrypt($data, $key, $expire = 0)
    {
        $key  = md5($key);
     ...
     * 字符串解密
     *
     * @param   string $data 要解密的字符串(必须是stringEncrypt方法加密的字符串)
     * @param   string $key  加密密钥
     * @see     stringEncrypt()
     * @return  string
     */
    public static function stringDecrypt($data, $key)
    {
        $key    = md5($key);
        $x      = 0;
     ...
     */
    protected static function encryption($str, $salt)
    {
        $str = base64_encode(self::randString(4, 5) . "." . $str . "." . self::randString(4, 5));
     ...
     * @return [type]      [description]
     */
    protected static function decryption($str, $salt)
    {
     ...
     * 自动转换字符集 支持数组转换
     *
     * @param  string|array $string 待转换的字符串或数组
